...
public class Verb {
	...
	private Word firstNPWord(Tree vp){
		List<Tree>children=vp.getChildrenAsList();
		for (Tree child: children){
			if (child.label().value().equalsIgnoreCase("NP")){
				Tree leaf=child.getLeaves().get(0);
				for (Word w: verb){
					if (w.getTree().equals(leaf)){
						return w;
					}
				}
			}
	...
	public String upperCammelCase() {
		...
		for (String s : verbWords) {
			if (!remToBe || !s.equalsIgnoreCase("be")){
