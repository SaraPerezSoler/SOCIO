...
public class History extends TelegramCommand {
	...
	private final String[] OPTIONS = new String[] { "\ud83d\udcb9 Statistics", "\ud83d\udcac Get messages", "Project history" };
	...
	public void commandAction(Update update) {
		...
		if (project == null || project.equals("")) {
			...
			String[][] optionsKeydoard = new String[][] { { OPTIONS[0] }, { OPTIONS[1] },{OPTIONS[2]}, { BACK } };
	...
	public void userAnswerText(Update update) {
		...
		if (start_state.get(update.getMessage().getChatId())) {
			...
			}else if (text.equals(OPTIONS[2])) {//Project History
				setStandardState(update.getMessage().getChatId());
				tChannel.write(update, CommandList.HISTORY_PROJECT, this.getProject(update.getMessage().getChatId()), "", "");
			} else {
				exit(update);
				return;
			}
