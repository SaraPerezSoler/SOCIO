...
public class DecisionMaker extends MainCommand {
	...
	public Response setChoice(ServletContext context, InputStream incomingData, Project actual, String branchGroup) throws Exception {
		...
		List<Project>group=actual.getOpenBranchsString().get(branchGroup);
		
		if (!user.isAdmin(actual)) {
			throw new InternalException("Only the project admin can select a branch");
		}
		if (group==null) {
			throw new InternalException("The project "+actual.getCompleteName()+" doesn't have the branch group "+branchGroup);
		}
		if (!group.contains(branch)) {
	...
	private File endDecision(ServletContext context, Project actual, String branchGroup, Project branchSelected) throws Exception {
		

		Decision d=actual.getDecision(branchGroup);
		if (d==null){
			throw new InternalException(branchGroup+" is not a close branch group in the project "+ actual.getCompleteName());
		}
		List<Project> p=d.getBranchs();
		if (!p.contains(branchSelected)){
			throw new InternalException(branchSelected.getCompleteName()+" is not a close branch in the group "+ d.getName());
		}
		
		File png=SocioData.getSocioData(context).makeDecision(actual, d, branchSelected, new Date());
		return png;
	}
	@POST
	...
	@Produces({ MediaType.APPLICATION_OCTET_STREAM, MediaType.TEXT_PLAIN })
	public Response consensus (@Context ServletContext context, InputStream incomingData,
			@PathParam("id") long id, @PathParam("branchGroup") String branchGroup) throws Exception {
		try {
	...
	private Response consensus (ServletContext context, InputStream incomingData,
			Project p, String branchGroup) {
