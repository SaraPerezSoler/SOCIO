...
public abstract class ProjectImpl extends MinimalEObjectImpl.Container implements Project {
	...
	private Stack<InteractionWithActions> undoMsg = new Stack<>();
	...
	@Override
	public File execute(RecommendationEvent msg) throws Exception {
		if (!isOpen()) {
			throw new InternalException("The project can't be edited. It is closed.");
		}
		if (isLocked()) {
		...
		List<Action> actions = addActions(msg);
		...
		if (actions.isEmpty()) {
			...
			undoMsg.clear();
			msg.setUndoable(true);
			msg.getCommands().addAll(actions);
			history.getMsg().add(msg);
		}
	...
	@Override
	public void execute(RecommendationMsg msg) throws Exception {
		if (!isOpen()) {
			throw new InternalException("The project can't be edited. It is closed.");
		}
		if (isLocked()) {
	...
	public abstract List<Action> addActions(RecommendationEvent objects) throws Exception;
	...
	private List<Action> getAllActions() {
		...
		for (UserInteraction m : history.getMsg()) {
			if (m instanceof InteractionWithActions) {
				actions.addAll(((InteractionWithActions) m).getAllActions());
			}
		}
		return actions;
	}
	...
	public File undo() throws Exception {
		...
		EList<UserInteraction> list = history.getMsg();

		ECollections.sort(list, new Comparator<UserInteraction>() {
			@Override
			public int compare(UserInteraction o1, UserInteraction o2) {
				return o2.getDate().compareTo(o1.getDate());
			}

		});
		for (UserInteraction ui : list) {
			if (ui instanceof Msg) {
				Msg m = (Msg) ui;
				if (m.isUndoable()) {
			...
			} else if (ui instanceof InteractionWithActions) {
				if (((InteractionWithActions) ui).isUndoable()) {
					List<Action> actions = ((InteractionWithActions) ui).getAllActions();
					if (actions != null) {
						for (int i = actions.size() - 1; i >= 0; i--) {
							Action a = actions.get(i);
							a.undoIt();
					...
					undoMsg.push((InteractionWithActions) ui);

					history.getMsg().remove(ui);
					break;
				}
			}
		}
	 ...
	 * @generated NOT
	 */
	public EList<UserInteraction> getHistoryMsg() {
	...
	public List<UserInteraction> getHistoryMsg(Date date, int order) {
		List<UserInteraction> history = getHistoryMsg();
		List<UserInteraction> ret = new ArrayList<>();
		if (order == DESCENDING) {
			Day aux = new Day(date);
			date = aux.getEnd();
			Collections.reverse(history);
			for (UserInteraction msg : history) {
				if (!msg.getDate().after(date)) {
					ret.add(msg);
				}
			}
		} else {
			for (UserInteraction rm : history) {
	 ...
	 * @generated NOT
	 */
	public List<UserInteraction> getHistoryMsg(Date start, Date end, int order) {
		if (end.before(start)) {
			Date aux = start;
			start = end;
			end = aux;
	 ...
	 * @generated NOT
	 */
	public List<UserInteraction> getHistoryForUser(User u) {
		List<UserInteraction> ret = new ArrayList<>();
		List<UserInteraction> list = getHistoryMsg();
		for (UserInteraction msg : list) {
	 ...
	 * @generated NOT
	 */
	public List<UserInteraction> getHistoryForElement(String element) {
		List<UserInteraction> ret = new ArrayList<>();
		List<UserInteraction> list = getHistoryMsg();

		for (UserInteraction msg : list) {
			if (msg instanceof InteractionWithActions) {
				if (((InteractionWithActions) msg).hasElement(element)) {
					ret.add(msg);
				}
			}
		}
	 ...
	 * @generated NOT
	 */
	public List<UserInteraction> getHistoryForAction(String action) {
		List<UserInteraction> ret = new ArrayList<>();
		List<UserInteraction> list = getHistoryMsg();
		for (UserInteraction msg : list) {
			if (msg instanceof InteractionWithActions) {
				if (((InteractionWithActions) msg).hasAction(action)) {
					ret.add(msg);
				}
			}
		}
	...
	public List<RecommendationMsg> getRecommendationMsg(){
		List<RecommendationMsg> ret = new ArrayList<RecommendationMsg>();
		for (UserInteraction ui: this.getHistory().getMsg()) {
			if (ui instanceof RecommendationMsg) {
				ret.add((RecommendationMsg)ui);
			}
		}
		return ret;
	}
	...
	private List<Action> blendProjects(Project p) throws Exception {
		...
		EList<UserInteraction> msgs = p.getHistoryMsg();
		List<Action> ret = new ArrayList<>();
		ECollections.sort(msgs, new Comparator<UserInteraction>() {
		...
		for (UserInteraction m : msgs) {
			UserInteraction m2 = m.getACopy();
