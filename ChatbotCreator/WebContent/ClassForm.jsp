...
<div class="row">
	<form action="">
		...
		<div class="row">
			<div class="col">
				<%if (selected.getAllAttribtues().isEmpty()){ %>
				<input type="radio" name="id" value="Attributes" disabled="disabled" onchange="enable_disable(this.value)"><span
					class="text">Identifiers Attributes</span>
				<%}else{%>
...
function enable_disable(value) {
	...
	if (value == "Attributes"){
		window.alert("Entra en attributes: "+value);
		<%for (NLAttribute attr : selected.getAllAttribtues()){%>
		document.getElementById("<%=attr.getName()%>").disabled = false;
		<%}%>
	}else if(value == "Default"){
		window.alert("Entra en default: "+value);
		<%for (NLAttribute attr : selected.getAllAttribtues()){%>
