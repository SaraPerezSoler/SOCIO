...
class RasaGenerator {
	...
	def doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		path = bot.name+"Rasa"
		fsa.generateFile(path + '/actions.py', actions(intents, entities, actions))
		fsa.generateFile(path + '/config.yml', config)
		fsa.generateFile(path + '/credentials.yml', credentials)
		fsa.generateFile(path + '/domain.yml', domain(intents, parameters, actions))
		fsa.generateFile(path + '/endpoints.yml', endpoint)
		fsa.generateFile(path + '/data/nlu.md', nlu(intents))
	...
	def actions(List<Intent> intents, List<Entity> entities, List<Action> actions)'''
	...
	class «action.name.rasaValue» (Action):
		...
		def run(self, dispatcher: CollectingDispatcher,
				tracker: Tracker,
				domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
			response = «action.HTTPRequest.name.rasaValue».response			
			text = «getHttpResponseText(action)»
	...
	'''
	def getHttpResponseText (HTTPResponse action){
		var ret = ""
		for (token : action.inputs.tokens){
			...
			if (!DialogflowGenerator.isTheLast(action.inputs.tokens, token)){
				ret += "+"
			}
		}
		return ret
	}
	def domain(List<Intent> intents, List<Parameter> parameters, List<Action> actions)'''
	intents:
	  «FOR intent: intents»
	...
	templates:
	  ...
	  «action.actionName»:
	  - text: «IF action.caption !== null»"«action.caption»"«ELSE»""«ENDIF»
	    image: "«action.URL»"
	  «ENDIF»
	  «ENDFOR»
	...
	def nlu(List<Intent> intents)'''
	«FOR intent: intents»
	## intent:«intent.name.getRasaValue»
	«FOR input: intent.inputs»
	- «input.generate()»
	«ENDFOR»
