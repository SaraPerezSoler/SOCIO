...
public class RoundImpl extends MinimalEObjectImpl.Container implements Round {
	...
	private Integer[] getDifference(List<Order> consensusOrder2, List<Order> order) {
		...
		for (int i=0; i<order.size(); i++){
			Order consensus=consensusOrder2.get(i);
			for (Order userOrder: order){
				if (consensus.getProjectName().equalsIgnoreCase(userOrder.getProjectName())){
					vector[i]=consensus.getPos()-userOrder.getPos();
					break;
				}
			}
	...
	public Map<User, Double> proximityMeasures(double beta){
		...
		for(Preference p: preferences){
			...
			for (Double prox: proxs){
				if (proxs[sol]!=prox){
					suma+=(1-prox)/(proxs.length-1);
				}
			}
			Double part1=(1-beta)*(suma);
			Double part2=beta*(1-proxs[sol]);
