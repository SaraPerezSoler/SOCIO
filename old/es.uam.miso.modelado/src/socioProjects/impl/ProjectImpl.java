...
public abstract class ProjectImpl extends MinimalEObjectImpl.Container implements Project {
	...
	public void remove() {
		...
		if (isBranch() && isOpen()) {
			...
			for (Decision d : getCloseBranchs()) {
	...
	public Project getFather() {
		if (!isOpen()) {
			if (eContainer() != null && eContainer().eContainer()!=null && eContainer().eContainer() instanceof Project) {
	...
	public List<Action> makeDecision(Decision d, Project branch) throws Exception {
		...
		if (d.getBranchs().contains(branch)) {
			...
			throw new SendMessageExc("The branch " + branch.getCompleteName() + " don't be in the branch " + d.getName()
					+ " of the project " + getCompleteName());
		}
	...
	public String getAllOpenBranchsGroupString() {
	...
	public Map<String, List<Project>> getAllOpenBranchsGroup() {
