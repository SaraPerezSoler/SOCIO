...
import es.uam.app.channels.telegramCommands.secrect.VoteAction;
...
public class TelegramControl extends TelegramLongPollingBot {
	...
	public void onUpdateReceived(Update update) {
		...
		if (update.hasMessage() && update.getMessage().hasText()) {
			boolean hasEntities= update.getMessage().hasEntities();
	...
	public void sendConsensusMessage(Msg msg, ConsensusMsg sMessage) {
		String id = msg.getId();
		String[] split = id.split(ID_SEPARATOR);
		int msgId = Integer.parseInt(split[0]);
		long chatId = Long.parseLong(split[1]);
		
		VoteAction va=SecretCommand.voteCommand;
		boolean isThisChannel=msg.getUser().getChannel().equalsIgnoreCase(getChannelName());
		va.modellingPrivateMsg(msgId, chatId, isThisChannel,  sMessage);

	}
	
	public void sendPrivateMessage(Msg msg, PrivateMsg e) {
		
		SendMessageExc aux= new  SendMessageExc("");
		for (User u: e.getUsers()){
			String next="\n"+e.getUserMsg(u);
			long id=u.getId();
			aux.setText(e.getUserMsg(u));
			TCommand state=TCommand.getStandardState();
			state.setStandardState(id);
			sendMessage(-1,id, aux);
		}
	}
