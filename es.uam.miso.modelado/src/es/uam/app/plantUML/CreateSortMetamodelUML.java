...
public class CreateSortMetamodelUML implements CreateText {
	...
	public static final String START_CLASS_DIAGRAM = START + "skinparam class {" + ENTR + "BackgroundColor" + TAG_START
			...
			+ "HeaderBackgroundColor" + TAG_START + CLASS_MOD + TAG_END + " " + OLD_BACKGROUND + ENTR + "BorderColor"
			+ TAG_START + CLASS_MOD + TAG_END + " " + OLD_LINES + ENTR + "}" + ENTR + "skinparam nodeSep 50 " + ENTR /////// configura
																														/////// espacio
																														/////// horizontal
			+ "skinparam rankSep 50 " + ENTR /////// configura espacio vertical
			+ "hide empty members   " + ENTR; /////// oculta la cajita de
												/////// atributos si está vacía
	...
	private Map<ClassControl, Boolean> prePrint = new HashMap<>();
	...
	private String getClassText(ClassControl cc, boolean print) {
		...
		// si se ha impreso antes, o se pretende imprimir ya la clase no es
		// necesario hacer nada con ella
		if (this.print.get(cc)!=null && this.print.get(cc)!=false){
			return "";
		}
		if (this.prePrint.get(cc)!=null && this.prePrint.get(cc)!=false){
			return "";
		}

		// Comprobamos si la clase s de las nuevas o de las antiguas
		String cad = "";
		boolean isOld = false;
		...
		// Creamos las posibles cabeceras de las clases, las clases antiguas llevan la etiqueta old y las clases 
		// antiguas con atributos nuevos llevan la etiqueda mod. Las clases nuevas no llevan etiqueta
		...
		//Se comprueba si hay algun atributo actual
		String attr = "";
		...
		//Con los atributos y las posibles opciones se crea la cabecera
		if (hasActualAttr && isOld) {
		...
		for (ClassControl superType : cc.getSuperTypes()) {
			...
			if (!hasNewTagUpdate(superType) && !hasActualTagUpdate(superType) && isOld == false) {
				this.prePrint.put(cc, true);
		...
		if ((isOld && print == false && hasActualAttr == false)) {
			return "";
		}
	...
	private boolean isActualAtr(AttributeControl ac) {
		...
		if (attr.contains(ac) || updateAttr.contains(ac) || addActualAttr.contains(ac)
				|| updateActualAttr.contains(ac)) {
			return true;
		}
