...
public class Voting extends SecretCommand {
	...
	public void commandAction(Update update) {
		...
		if (split.length != 2) {
			...
			this.tChannel.sendMessage(-1, chatId, sMessage);
		} else {
	...
	public void modellingAnswer(long chatId, int msgId, Msg rMessage, SendMessageExc sMessage) {
		if (privateState.get(chatId) == START){
			...
			if (options.length==0){
				...
				tChannel.sendMessageWithKeyBoar(-1, chatId, sMessage, options);
			}
	...
	public void userAnswerText(Update update) {
		...
		if (privateState.get(chatId) == START) {
			...
			tChannel.sendMessageWithKeyBoar(-1, chatId, sMsg, new String[][]{{OPTIONS[0]},{OPTIONS[1]},{OPTIONS[2]}});
			...
			if (update.getMessage().getText().equals(END)) {
				if (users.get(chatId) == null || users.get(chatId).size() < 2) {
					...
					tChannel.sendMessage(-1, chatId, sMessage);
					askForUsers(update);
				} else {
	...
	private void askForUsers(Update update) {
		long chatId = update.getMessage().getChatId();
		SendMessageExc sMessage = new SendMessageExc(
				"Who should vote? (channel/nick). Or press end if already send all the users.");
		tChannel.sendMessageWithKeyBoar(-1, chatId, sMessage, new String[][] { { END } });
	}
}
